...
add_or_update=None #教师当前是在修改成绩页还是录入成绩页,true为add,false为update
...
class Admin_MenuWindow(Admin_Menu_Ui,QtWidgets.QMainWindow):
    ...
    switch_window2=QtCore.pyqtSignal() # 跳转信号 用于瑞出
    def __init__(self):
...
class CourseDetailWindow(CourseDetail_Ui,QtWidgets.QMainWindow):
    ...
    def update_grade(self,info):
        global gradeStudent,add_or_update
    ...
    def delete_grade(self,info):
        result=show_msg_box("删除成绩", "确定删除该学生的成绩吗？")
        if result=="OK":
            print("OK")
            # 创建游标
...
# 添加学生窗口
class AddStudentWindow(AddStudent_Ui,QtWidgets.QMainWindow):
    def __init__(self):
...
class Teacher_add_gradeWindow(Teacher_add_grade_Ui,QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setupUi(self)
        global thisConnect,thisUsrNo,courseInformations,courseInformation,gradeStudent,add_or_update
        if(add_or_update==False):#如果是修改成绩页
